...
### Mock-data helpers (no API traffic)

```bash
npm run dev:mock      # Starts remix dev with fixtures/dashboard.sample.json
npm run start:mock    # Builds + serves the mocked data set
```
...
npm run check       # Convenience (typecheck + vitest)

npm run cy:open     # Interactive Cypress tests (requires npm run dev:mock)
npm run test:e2e    # Headless Cypress (auto-starts mock dev server)
```

Vitest covers utility logic and UI components, while Cypress drives the dashboard end-to-end using the local fixture instead of the live API.
